...
function getAuthenticatedClient() {
    return new Promise((resolve, reject) => {
        ...
        const oAuth2Client = new OAuth2Client(keys.web.client_id, keys.web.client_secret, keys.web.redirect_uris[1]);
        ...
        const server = http
            ...
            openAux(authorizeUrl, { wait: false }).then((cp) => cp.unref());
        });
        destroyer(server);
    });
}
